...
**Назначение:** класс для хранения и валидации данных покупателя при оформлении заказа.
...
- `isValidStep1(): boolean` — проверяет валидность данных первого шага (оплата и адрес).
- `isValidStep2(): boolean` — проверяет валидность данных второго шага (email и телефон).
- `isValid(): boolean` — выполняет полную валидацию всех данных.
